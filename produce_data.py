...
def _get_sdf(file_in, file_out, fix_sample_cnt):
    ...
    fix_surf_pnts = np.squeeze(
        fix_surf_pnts)  # add .copy() to this line and change variable name if previous line has more use down the line
...
def _fix_sampling(nals_file, nmesh_file, file_out, fix_sample_cnt):
    ...
    if len(normalized_data) > fix_sample_cnt:
        ...
        while remainder != 0:
            ...
            if (len(normalized_data) + len(k_grp_centers)) > fix_sample_cnt or (
                    len(normalized_data) + len(k_grp_centers)) == fix_sample_cnt:
...
def main():
    dataset_dir = "./data"
    num_processes = 8
    fix_sample_cnt = '2048' # 1024, 2048, 3072, 4096

    data_folders = os.listdir("./data/all_data")

    for data_folder in tqdm(data_folders):
        if data_folder not in ['Building']:
        #if data_folder != 'Hiiumaa':
            continue
        ...
        for mesh_file in mesh_list:
            if mesh_file[:-4]+'.xyz' not in xyz_list:
        ...
        clean_meshes(in_dir=f'{df_dir}/mesh', out_dir="processed/%s/02_cleaned_xyz"%(fix_sample_cnt),
