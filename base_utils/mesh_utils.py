...
def laplacian_smooth(verts, mesh_or_faces, iterations=1, lam=0.5):
    # 如果传的是 Mesh，就取出 faces
    if hasattr(mesh_or_faces, "faces"):
        faces = mesh_or_faces.faces
    else:
        faces = mesh_or_faces
    ...
    for f in faces:
        if f.max() >= N:  # 防御性检查
            continue
        adjacency[f[0].item()].extend([f[1].item(), f[2].item()])
        adjacency[f[1].item()].extend([f[0].item(), f[2].item()])
        adjacency[f[2].item()].extend([f[0].item(), f[1].item()])
    adjacency = [list(set(neigh)) for neigh in adjacency]

    smoothed_verts = verts.clone()
    for _ in range(iterations):
        new_verts = smoothed_verts.clone()
        for i, neigh in enumerate(adjacency):
            if len(neigh) > 0:
                neighbor_mean = smoothed_verts[neigh].mean(dim=0)
                new_verts[i] = smoothed_verts[i] * (1 - lam) + neighbor_mean * lam
