...
from loss_pcc import chamfer_loss_sqrt, l2_normal_loss, density_cd, get_loss1, chamfer_loss, patch_residual_loss
...
pcc_model = PCCNet(kmax=30, code_dim=256).to(device)  #1024


if args.load_chkpnt:
    optimizer = torch.optim.AdamW(pcc_model.parameters(), lr=args.fix_lr, betas=(0.9, 0.999),weight_decay=args.wd)  #eps=1e-08,
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.88, patience=1, verbose=True)
else:
    optimizer = torch.optim.AdamW(pcc_model.parameters(), lr=args.lr, betas=(0.9, 0.999),weight_decay=args.wd)  #eps=1e-08,

    num_steps = args.max_epoch
...
llr_logger.info('ms:[10,20,30] | 256-trsf | 4heads | scale: x4 | bs: 8 | #ep: 140 |lr: 0.0002 | eta_min: 1e-07 | normals: Yes | #tr/ts: 2k/40 | tr_loss: %s' %(args.tr_loss))
...
max_epoch = init_epoch + 65  if args.load_chkpnt else args.max_epoch
best_val_cdp = 20.0
best_val_cdt = 20.0
coarse_list, fine_list, finer_list,total_list = [], [], [],[]

for epoch in tqdm(range(init_epoch, max_epoch+1)):
    avg_time_per_iter = []
    for i, data in enumerate(tr_loader, 0):
        ...
        gt_xyz = data[1][:, :, :6].to(device).float()

        gt_fine = p2u.gather_operation(gt_xyz.permute(0,2,1).contiguous(), p2u.furthest_point_sample(gt_xyz[:,:,:3].contiguous(), fine.size(1))).permute(0,2,1).contiguous()
        ...
        loss = loss_coarse.mean() + loss_fine.mean() 
