...
def gather_features(features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
    B, C, N = features.shape
    _, M = idx.shape

    idx = idx.long()  # ✅ 确保为 int64
    idx_expanded = idx.unsqueeze(1).expand(-1, C, -1)  # [B, C, M]

    return torch.gather(features, dim=2, index=idx_expanded)
class CustomDataset(Dataset):

    def __init__(self, split='train', npoints=2048, test_split_cnt=40, device='cuda'):
        self.split = split
        self.device = device
        ...
        self.a2p_il_path = './ablations/a2p_li.txt'
        self.cache_dir = "./data/processed/2048/cached_complete_npy_trimesh"
        self.complete_cache = {}
        self.complete_cache_test = {}
        ...
        mesh_filelist = os.listdir(self.mesh_path)
        test_data_list = ['Building']
        #test_data_list = ['Tartu1', 'Tartu2', 'Tartu3']
        #test_data_list = ['building_building_37','building_building_38','building_building_42','building_building_43','building_building_52']
        ...
        if self.split == 'train' or self.split == 'custom' or (len(self.split.split('_'))>1 and self.split.split('_')[1] == 'tr'):
            self.mesh_list = self.tr_mesh_list
            self.partial_list = self.tr_partial_list

                #print("All meshes preprocessed and saved to GPU cache.")
        elif len(self.split.split('_'))>1 and self.split.split('_')[1] == 'ts':
            self.mesh_list = self.tr_mesh_list
            self.partial_list = self.tr_partial_list
    ...
    def __getitem__(self, index):
        ...
        # 2) load cached complete
        # cache_file = os.path.join(self.cache_dir, base + '_complete.npy')
        # complete_pc = np.load(cache_file)  # (8192,6)
        ...
        fps_idx = pointnet2_utils.furthest_point_sample(surf_pnt_samples, 16384)  # xyz: torch.Tensor
        ...
        complete_normals = pointnet2_utils.gather_operation(surf_pnt_normals.permute(0, 2, 1).contiguous(),
                                                            fps_idx.int())
        # t.toc('mesh/fps sampling + indexing took') #Time elapsed since t.tic()
        complete_pc = torch.cat((torch.squeeze(complete_pc), torch.squeeze(complete_normals)), 0).permute(1,
                                                                                                          0)  # add .copy() to this line and change variable name if previous line has more use down the line
        ...
        cache_file = os.path.join(self.cache_dir, base + '.npz')
        data = np.load(cache_file)
        complete_pc = data['complete']  # shape (16384, 6)

        # 3) to torch
        partial_tensor = torch.from_numpy(partial_pc).float().to(self.device)  # [N, 6]
        complete_tensor = torch.from_numpy(complete_pc).float().to(self.device)  # [16384, 6]
        

        return partial_tensor, complete_tensor
         '''
