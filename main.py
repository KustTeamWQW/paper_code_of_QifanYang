...
from config_a2p import Args as args
...
os.makedirs(os.path.join('./data/processed/%s/net_outputs/p2m_rec_obj', 'config-a2pil{}'.format(exp_num)),
            exist_ok=True)  # config details are in matching loss_log file.
...
rebt_files = glob.glob(f'./data/processed/2048/net_outputs/pcc_out/a2p_il_fine/*.txt')
#print(rebt_files)
...
for pc_filename in rebt_files:
    ...
    file_chk = os.path.join('./data/processed/2048/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}',
                            f'last_rec_{pc_fname_no_ext}.obj')
    ...
    initial_data = np.loadtxt(os.path.join(f'./data/processed/2048/net_outputs/pcc_out/a2p_il_fine', pc_filename))
    ...
    if args.initial_mesh:
        ...
        remeshed_vertices, remeshed_faces = mesh_utils.remesh(convex_hull,
                                                              # TODO: remove normalization, to ensure chamfer dist coa2pilectness
                                                              args.initial_num_faces)
    mesh_utils.save("%s/%s_initial_mesh.obj" % (
    os.path.join('./data/processed/2048/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}'), pc_fname_no_ext),
                    remeshed_vertices, remeshed_faces)

    # initial mesh
    mesh = Mesh("%s/%s_initial_mesh.obj" % (
    os.path.join('./data/processed/2048/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}'), pc_fname_no_ext),
                device=device, hold_history=True)
    ...
    rand_verts = mesh_utils.populate_e(
        [mesh])  # totally random verticies re-indexed with mesh edges. NB: 6 = a pair 3D random coords of an egde
    ...
    for i in range(args.iterations):
        ...
        for part_i, est_verts in enumerate(model(rand_verts, part_mesh)):
            ...
            num_samples = get_num_samples(args, i % args.upsamp)
            recon_xyz, recon_normals = mesh_utils.sample_surface(part_mesh.main_mesh.faces,
            ...
            xyz_chamfer_loss, normals_chamfer_loss = chamfer_distance(recon_xyz, input_xyz,
                                                                      # replaced input_xyz with pcc_xyz_as_gt
            ...
            if (i < args.beamgap_iterations) and (i % args.beamgap_modulo == 0):
                ...
                loss = (xyz_chamfer_loss + (
                            args.ang_wt * normals_chamfer_loss))  # this n the next loss component might be related to normal consistency loss
        ...
        if i % args.export_interval == 0 and i > 0:
            ...
            with torch.no_grad():
                part_mesh.export(os.path.join('./data/processed/%s/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}',
                                              f'rec_{pc_fname_no_ext}.obj'))
            mesh_path = os.path.join('./data/processed/%s/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}',
                                     f'rec_{pc_fname_no_ext}.obj')
        ...
        if (i > 0 and (i + 1) % args.upsamp == 0):
            ...
            if num_faces > len(mesh.faces) or args.manifold_always:
                # up-sample mesh
                mesh = mesh_utils.manifold_upsample(mesh,
                                                    f'./data/processed/%s/net_outputs/p2m_rec_obj/config-a2pil{exp_num}',
                                                    pc_fname_no_ext, Mesh,
    ...
    with torch.no_grad():
        mesh.export(os.path.join('./data/processed/%s/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}',
                                 f'last_rec_{pc_fname_no_ext}.obj'))

print('p2m done...!')

# compute final losses
rlist = glob.glob(os.path.join('./data/processed/%s/net_outputs/p2m_rec_obj', f'config-a2pil{exp_num}/rec_*.obj'))
print(rlist)
