...
def get_graph_feature(x, k1=20, k2=20, idx=None, Norm_sample=False):
    batch_size = x.size(0)
    num_points = x.size(2)
...
def my_get_graph_feature(x, feature, k1=20, k2=20, use_x=False):
    batch_size = x.size(0)
    num_points = x.size(2)
    x = x.contiguous().view(batch_size, -1, num_points)  # [B, 3, N]

    idx = knn(x, k1=k1, k2=k2).contiguous()
    ...
    _, num_dims, _ = feature.shape

    feature = feature.transpose(2, 1).contiguous()  # [B, N, C]

    try:
        feature = feature.view(batch_size * num_points, -1)[idx, :].contiguous()
    except:
        import ipdb
        ipdb.set_trace()
        print(feature.shape)

    feature = feature.view(batch_size, num_points, k1, num_dims)  # [B, N, k, d]

    if use_x:
        x = x.transpose(1, 2).contiguous().view(batch_size, num_points, 1, 3).repeat(1, 1, k1, 1)
        return torch.cat((x, feature), -1).contiguous().view(-1, k1, num_dims+3)
    else:
        return feature.contiguous().view(-1, k1, num_dims)
