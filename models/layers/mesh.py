...
from base_utils.mesh_utils import load, export
import copy
...
class Mesh:
    ...
    def __init__(self, file, hold_history=False, vs=None, faces=None, device='cpu', gfmm=False, nml=True):
        if file is None:
            return
    ...
    def build_gemm(self):
        ...
        self.vei = torch.from_numpy(np.concatenate(self.vei).ravel()).to(self.device).long() # removed np.array from np.array(self.vei)
        # self.vei = torch.Tensor(self.vei).to(self.device).long()
        self.nvsi = torch.Tensor(np.concatenate(self.nvsi).ravel()).to(self.device).long()  # removed np.array from np.array(self.nvsi)
        # self.nvsi = torch.Tensor(self.nvsi).to(self.device).long()
        self.nvsin = torch.from_numpy(np.concatenate(self.nvsin).ravel()).to(self.device).long() # removed np.array from np.array(self.nvsin)
        # self.nvsin = torch.Tensor(self.nvsin).to(self.device).long()
        ve_in = copy.deepcopy(self.ve)
        self.ve_in = torch.Tensor(np.concatenate(ve_in).ravel()).to(self.device).long() # removed np.array from np.array(ve_in)
    ...
    def deep_copy(self): #TODO see if can do this better
        ...
        for attr in self.__dir__():
            ...
            if type(val) == types[0]:
                new_mesh.__setattr__(attr, val.copy()) #set
...
class PartMesh:
    ...
    @staticmethod
    def create_submesh(vs_index: torch.Tensor, mesh: Mesh) -> Tuple[Mesh, torch.Tensor]:
        """
